...
async function launchBrowser(headless) {
  ...
  const macExecutableCandidates = browserRootCandidates.flatMap((browserRoot) => [
    path.join(browserRoot, "chromium-1217", "chrome-mac-arm64", "Google Chrome for Testing.app", "Contents", "MacOS", "Google Chrome for Testing"),
    path.join(browserRoot, "chromium-1217", "chrome-mac-x64", "Google Chrome for Testing.app", "Contents", "MacOS", "Google Chrome for Testing"),
  ]);
  
  // Windows executable paths
  const winExecutableCandidates = browserRootCandidates.flatMap((browserRoot) => [
  ...
  const linuxExecutableCandidates = browserRootCandidates.flatMap((browserRoot) => [
    path.join(browserRoot, "chromium-1217", "chrome-linux", "chrome"),
  ]);
  
  // Combine all candidates
  const executableCandidates = [
    ...macExecutableCandidates,
    ...winExecutableCandidates,
    ...linuxExecutableCandidates
  ];
  ...
  try {
    ...
    throw new Error(
      `Failed to launch Playwright Chromium. Install the browser payload or extract ms-playwright-browsers.zip to the Playwright cache. Checked: ${executableCandidates.slice(0, 5).join(", ")}... Original error: ${error.message}`,
    );
  }
}
