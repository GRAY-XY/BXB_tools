...
const MACOS_RELEASE_TITLE_PREFIX = "BXB Homework v";
...
function extractMacOSReleaseVersion(release) {
  const title = String(release?.name || "").trim();
  const version = title.startsWith(MACOS_RELEASE_TITLE_PREFIX)
    ? title.slice(MACOS_RELEASE_TITLE_PREFIX.length).trim()
    : null;
  if (!version) return null;
  return parseAppVersion(version);
}

function chooseMacOSInstallerAsset(release, version) {
  const assets = Array.isArray(release?.assets) ? release.assets : [];
  const installers = assets.filter((asset) => {
    const name = String(asset?.name || "").toLowerCase();
  ...
  return (
    installers.find((asset) =>
      String(asset?.name || "").toLowerCase().includes(normalizedVersion)) ||
    installers[0]
  );
}

function getPlatformReleaseVersion(release) {
  if (process.platform === "darwin") {
    return extractMacOSReleaseVersion(release);
  }
  return extractWindowsReleaseVersion(release);
}

function choosePlatformInstallerAsset(release, version) {
  if (process.platform === "darwin") {
    return chooseMacOSInstallerAsset(release, version);
  }
  return chooseWindowsInstallerAsset(release, version);
}

function getPlatformUpdateChannel() {
  return process.platform === "darwin" ? "macOS stable" : "Windows stable";
}
...
function assertUpdateCacheFile(filePath) {
  ...
  const allowedExtensions = process.platform === "darwin" ? [".dmg"] : [".exe"];
  const ext = path.extname(resolved).toLowerCase();
  if (!allowedExtensions.includes(ext)) {
    throw new Error(`更新安装器必须是 ${allowedExtensions.join(" 或 ")} 文件。`);
  }
  return resolved;
}
...
async function ensurePlaywrightBrowsers() {
  ...
  if (installResult.status !== 0) {
    throw new Error(`Playwright 浏览器安装失败：退出码 ${installResult.status}`);
  }

  const status = getBrowserDependencyStatus();
  if (!status.ready) {
    throw new Error(`Playwright 浏览器已安装，但没有找到 Chromium 可执行文件。安装目录：${browserRoot}`);
  }
  return status;
}
...
async function checkForUpdates() {
  ...
  try {
    ...
    const candidates = (Array.isArray(releases) ? releases : [])
      ...
      .map((release) => ({ release, version: getPlatformReleaseVersion(release) }))
    ...
    if (!latest) {
      return {
        ...
        hasUpdate: false,
        message: `No ${getPlatformUpdateChannel()} release was found.`,
    ...
    return {
      ...
      sha256Asset: sha256Asset
        ...
        : null,
      releasesUrl: RELEASES_PAGE_URL,
      message: hasUpdate ? `Found ${latest.release.name}.` : `Already on the latest ${getPlatformUpdateChannel()} version.`,
    };
  } catch (error) {
    return {
      ok: false,
