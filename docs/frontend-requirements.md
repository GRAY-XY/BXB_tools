...
- Draft formulas must use ordinary keyboard notation that resembles manually typed student work: `x^2`, `sqrt(x)`, `*`, `/`, `<=`, `theta`, `x1`, `vmax`, and `21.8 度/degrees`. Do not generate typeset superscripts, subscripts, roots, multiplication/division signs, inequality/approximation signs, arrows, Greek glyphs, or degree signs unless the task explicitly requires one.
- Generation 08 G8A is the accepted production draft-writing baseline. It preserves the natural keyboard-notation baseline, keeps the original task order and format, sizes answers according to command verbs, resolves classifications from definitions supplied in the task, and retains full numerical precision until final rounding. Future prompt iterations must be evaluated against it and must not regress into typeset symbols, incorrect rounding, category drift, unnecessary length, tutorial-style prose, or missing scoring evidence.
- Prompt-evaluation Generations 06 and 07 are archived experimental failures and are not part of the accepted lineage because they inherited an incorrect expected rounding result.
